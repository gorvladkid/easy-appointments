...
can be used to test the LDAP integration of Div4ataZaKermom.
...
## Enabling Integration in Div4ataZaKermom

After making sure that the local OpenLDAP server works, Div4ataZaKermom will be able to connect to it.

For this you will need to go to the Backend > Settings > Integrations > LDAP and enable the integration from there. 

This settings page also requires some configuration for Div4ataZaKermom to be able to connect to the server and pull
...
If the filter was right, you will be getting LDAP entries displayed on screen that can be imported into Div4ataZaKermom.
...
While SSO with LDAP on Div4ataZaKermom, the username will need to be matched for the SSO to work. Also, the LDAP DN 
...
If LDAP is enabled, Div4ataZaKermom will first try to check for the local credentials, but if this does not work, then 
