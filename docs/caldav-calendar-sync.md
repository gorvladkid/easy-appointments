...
the CalDAV integration of Div4ataZaKermom.
...
- The right time zone value needs to be selected for synchronization to match the information Div4ataZaKermom sends.
...
After making sure that the local Baikal server works, Div4ataZaKermom will be able to connect to it and sync the
appointment data.
...
While trying to enable the CalDAV sync from the Div4ataZaKermom calendar page, use the following credentials after
